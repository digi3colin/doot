package doot.motion {
	...
	import flash.utils.setTimeout;
		...
		private function setDeltaValues() : void {						deltaState = new MotionTransform();			deltaState.x=targetState.x  -beginState.x;
			...
			dispatchEvent(new Event(MotionEvent.EVENT_TWEENING));//{target:this,type:'onTween'});
			time++;
			if(time>dur){				killTween();
				onTweenEnd();
			}
		...
		private function onTweenEnd():void{
			dispatchEvent(new Event(MotionEvent.EVENT_END));
		}
	
		private function onTweenStart():void{
			dispatchEvent(new Event(MotionEvent.EVENT_START));
		}
